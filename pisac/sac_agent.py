...
import gin
import numpy as np
from pisac import tanh_normal_projection_network
...
@gin.configurable
def sac_normal_projection_net(action_spec):
  """SAC version."""
...
class SacAgent(tf_agent.TFAgent):
  ...
  def __init__(self,
               ...
               log_alpha_in_alpha_loss=True,
               target_entropy=None,
               heuristic_target_entropy_type='-dim(A)/2',
    ...
    Args:
      ...
      log_alpha_in_alpha_loss: using log_alpha of alpha in alpha_loss.
      ...
      heuristic_target_entropy_type: type of heuristic target entropy
      gradient_clipping: Norm length to clip gradients.
      train_sequence_length: 2 if using single step transition.
      use_target_obs: A bool to use a different observation input for targert
       critic in the critic_loss_q_aug() function.
      debug_summaries: A bool to gather debug summaries.
      summarize_grads_and_vars: If True, gradient and network variable summaries
        will be written during training.
      train_step_counter: An optional counter to increment every time the train
        op is run.  Defaults to the global_step.
      auto_step: agent controls train_step_counter increment.
    ...
    if target_entropy is None:
      target_entropy = self._get_default_target_entropy(
          action_spec, heuristic_target_entropy_type)

    self.log_alpha_in_alpha_loss = log_alpha_in_alpha_loss
    ...
    self._update_target = self._get_target_updater(
        tau=self.target_update_tau, period=self.target_update_period)
    self.auto_step = auto_step
    self.use_target_obs = use_target_obs
  ...
  def _get_default_target_entropy(self, action_spec,
    ...
    if heuristic_target_entropy_type == '-dim(A)/2':
      ...
      raise NotImplementedError(
          'Unknown type {}'.format(heuristic_target_entropy_type))
  ...
  def experience_to_transitions(self, experience):
  ...
  def _train(self, experience, weights):
    ...
    Returns:
      SAC loss info
    """
    time_steps, actions, next_time_steps = self.experience_to_transitions(
        experience)
    ...
    with tf.GradientTape(watch_accessed_variables=False) as tape:
      ...
      actor_loss = self._actor_loss_weight * actor_loss
  ...
  def critic_loss(self,
    ...
    Args:
      ...
      target_obs: Optional target observation.
    ...
    with tf.name_scope('critic_loss'):
      ...
      if target_obs is None:
        target_obs = next_time_steps.observation

      target_input = (target_obs, next_actions)
  ...
  def actor_loss(self, time_steps, next_time_steps=None, weights=None):
    """Computes the actor_loss for SAC training.

    Args:
      time_steps: A batch of timesteps.
      next_time_steps: The next timesteps.
      weights: Optional scalar or elementwise (per-batch-entry) importance
        weights.
    ...
    with tf.name_scope('actor_loss'):
      ...
      critic_obs = time_steps.observation
      target_input = (critic_obs, actions)

      target_q_values1, _ = self._critic_network_1(
          target_input, time_steps.step_type, training=True)
  ...
  def alpha_loss(self, time_steps, weights=None):
    ...
    with tf.name_scope('alpha_loss'):
      ...
      if self.log_alpha_in_alpha_loss:
      ...
      self._alpha_loss_debug_summaries(alpha_loss, entropy_diff, log_pi)
  ...
  def _alpha_loss_debug_summaries(self, alpha_loss, entropy_diff, log_pi):
    if self._debug_summaries:
  ...
  def critic_loss_q_aug(self,
                        time_steps,
                        actions,
                        next_time_steps,
                        td_errors_loss_fn,
                        target_obs=None,  # target critic input
                        augmented_obs=None,
                        augmented_next_obs=None,
                        augmented_target_obs=None,
                        gamma=1.0,
                        reward_scale_factor=1.0,
                        weights=None,
                        training=False):
    ...
    with tf.name_scope('critic_loss'):
      ...
      td_targets = self._compute_td_targets(
          next_time_steps, reward_scale_factor, gamma, target_obs=target_obs)

      if augmented_next_obs is not None:
        augmented_next_time_steps = next_time_steps._replace(
            observation=augmented_next_obs)
        augmented_td_targets = self._compute_td_targets(
            augmented_next_time_steps, reward_scale_factor, gamma,
            target_obs=augmented_target_obs)
        td_targets = (td_targets + augmented_td_targets) / 2.0

      obs = time_steps.observation
      pred_input = (obs, actions)
      pred_td_targets1, pred_td_targets2, critic_loss = (
          self._compute_prediction_critic_loss(
              pred_input, td_targets, time_steps, training, td_errors_loss_fn))

      if augmented_obs is not None:
        augmented_pred_input = (augmented_obs, actions)
        _, _, augmented_critic_loss = (
            self._compute_prediction_critic_loss(
                augmented_pred_input, td_targets, time_steps,
                training, td_errors_loss_fn))
        critic_loss += augmented_critic_loss
  ...
  def train_pix(self, time_steps, actions, next_time_steps, x0, x1, x0a=None,
                x1a=None, e_enc=None, e_enc_t=None, q_aug=False,
                use_critic_grad=True, weights=None):
    """Train with pixel observation encoders from transitions.

    Note that this method supports DrQ style Q-augmentation which is not used
    in standard PI-SAC.

    Args:
      time_steps: TimeStep namedtuple representing a batch of steps.
      actions: batched action tuple.
      next_time_steps: Next TimeStep namedtuple representing a batch of steps.
      x0: pixel observation of current step.
      x1: pixel observation of next step.
      x0a: (Optional) pixel observation of current step in a different view.
      x1a: (Optional) pixel observation of next step in a different view.
      e_enc: observation encoder.
      e_enc_t: target observation encoder.
      q_aug: A boolean. If true, perform DrQ style Q augmentation.
      use_critic_grad: A boolean. If true, backprop critic gradients to encoder.
      weights: Optional scalar or elementwise (per-batch-entry) importance
        weights.
    ...
    Returns:
      SAC loss info
    """
    trainable_critic_variables = self._critic_network_1.trainable_variables
    trainable_critic_variables += self._critic_network_2.trainable_variables
    if use_critic_grad:
      trainable_critic_variables += e_enc.trainable_variables
    with tf.GradientTape(watch_accessed_variables=False) as tape:
      assert trainable_critic_variables, ('No trainable critic variables to '
                                          'optimize.')
      tape.watch(trainable_critic_variables)
      feat_x0, _ = e_enc(x0)
      feat_x1, _ = e_enc(x1)
      feat_x1_target, _ = e_enc_t(x1)
      time_steps = time_steps._replace(observation=feat_x0)
      next_time_steps = next_time_steps._replace(observation=feat_x1)
      if q_aug:
        augmented_feat_x0, _ = e_enc(x0a)
        augmented_feat_x1, _ = e_enc(x1a)
        augmented_feat_x1_target, _ = e_enc_t(x1a)

      critic_loss = self._critic_loss_weight*self.critic_loss_q_aug(
          time_steps,
          actions,
          next_time_steps,
          td_errors_loss_fn=self._td_errors_loss_fn,
          target_obs=feat_x1_target,
          augmented_obs=augmented_feat_x0 if q_aug else None,
          augmented_next_obs=augmented_feat_x1 if q_aug else None,
          augmented_target_obs=augmented_feat_x1_target if q_aug else None,
          gamma=self._gamma,
          reward_scale_factor=self._reward_scale_factor,
          weights=weights,
          training=True)
    ...
    with tf.GradientTape(watch_accessed_variables=False) as tape:
      assert trainable_actor_variables, ('No trainable actor variables to '
                                         'optimize.')
      tape.watch(trainable_actor_variables)
      actor_loss = self.actor_loss(time_steps, next_time_steps, weights=weights)
      actor_loss = self._actor_loss_weight * actor_loss
    tf.debugging.check_numerics(actor_loss, 'Actor loss is inf or nan.')
    actor_grads = tape.gradient(actor_loss, trainable_actor_variables)
    self._apply_gradients(actor_grads, trainable_actor_variables,
